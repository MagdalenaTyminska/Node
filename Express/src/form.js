...
app.use(express.urlencoded({ extended: false })); // dodany middleware żeby otrzymać dane, parser danych z formularzy
...
app.post("/login", (req, res) => {
  const data = req.body; //body - klucz i wartość, najczęściej json
...
// app.get("/users", (req, res) => res.json({ users: users.map(({pwd, ...user}) => user) }));
